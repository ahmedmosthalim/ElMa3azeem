...
class CancelReasonsVC: BottomPopupViewController {
    enum cancelType {
        case user
        case delegate
        case provider
...
extension CancelReasonsVC: UITableViewDelegate, UITableViewDataSource {
...
extension CancelReasonsVC {
    ...
    func cancelOrder(orderID: Int, reasone: String) {
        showLoader()
        CreateOrderNetworkRouter.cancelOrder(orderID: String(orderID), reason: reasone).send(GeneralModel<UserModel>.self) { [weak self] result in
            guard let self = self else { return }
            self.hideLoader()
            switch result {
            case let .failure(error):
